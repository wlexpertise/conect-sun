...
def carregar_dados():
    ...
    col_tipo_p = df.columns[15]        # Coluna P - Tipo real para batimento
...
# 🎯 AJUSTE: O selectbox agora abre por padrão no último mês disponível da lista (len - 1)
...
if pagina == "🚀 Visão Geral (YTD)":
    ...
    # 📌 NOVOS CARDS: Dados do Mês Selecionado
    ...
    m3.metric("📊 Resultado Líquido Mensal", formatar_brl(res_mes), delta=f"{formatar_pct(margem_mes)} Margem")

    st.markdown("---")
    
    # 📌 CARDS ORIGINAIS: Dados Acumulados (YTD)
